...
function App() {
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      // Ensure user has required arrays for dashboard stats
    ...
    // Load courses from backend or fallback to default
    const loadCourses = async () => {
      try {
        const coursesData = await coursesAPI.getAll();
        setCourses(coursesData);
      } catch (error) {
        console.error('Backend not available, using default courses');
    ...
    <Router future={{ v7_startTransition: true, v7_relativeSplatPath: true }}>
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
        ...
        <Routes>
          ...
          <Route 
            path="/dashboard" 
            element={user ? <Dashboard user={user} userProgress={userProgress} /> : <Navigate to="/auth" />} 
          />
          <Route 
            path="/courses" 
            element={user ? <Courses courses={courses} user={user} setUser={setUser} setUserProgress={setUserProgress} /> : <Navigate to="/auth" />} 
          />
          <Route 
            path="/learning/:courseId" 
            element={user ? <Learning courses={courses} user={user} userProgress={userProgress} setUserProgress={setUserProgress} /> : <Navigate to="/auth" />} 
          />
          <Route 
            path="/assignment/:courseId" 
            element={user ? <Assignment courses={courses} user={user} setUser={setUser} setUserProgress={setUserProgress} /> : <Navigate to="/auth" />} 
          />
          <Route 
            path="/certificate/:courseId" 
            element={user ? <Certificate courses={courses} user={user} setUser={setUser} /> : <Navigate to="/auth" />} 
          />
          <Route 
            path="/progress" 
            element={user ? <Progress user={user} courses={courses} userProgress={userProgress} /> : <Navigate to="/auth" />} 
          />
          <Route 
            path="/certificates" 
            element={user ? <Certificates user={user} courses={courses} /> : <Navigate to="/auth" />} 
          />
